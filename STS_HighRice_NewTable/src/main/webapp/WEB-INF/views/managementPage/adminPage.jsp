...
$(function(){
      ...
      $("#product_grid").jsGrid({
           ...
           fields: [
              ...
              {name:"deleteCondtion",title:"수정", width:100, itemTemplate:function(_,item){
                 var str = $("<center></center>").html("<button>수정</button>").on("click",function(){
                    $("#productDialog").dialog("open")
                       $("#product_name").val(item.product_name)
                       $("#product_id").val(item.product_id)
                       $("#category").val(item.category)
                       $("#main_img").val(item.main_img)
                       $("#sub_img").val(item.sub_img)
