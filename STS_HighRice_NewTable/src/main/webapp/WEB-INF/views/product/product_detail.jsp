...
//       })
      ...
      $.ajax({
         ...
         success:function(data){
            ...
            var regdateDiv = $("<div></div>").html(data.regdate);
            var contentDiv = $("<textarea id='content_up'>"+data.content+"</textarea>")
            var updateBtnDiv = $("<input type='button' value='수정'>").click(function(){
            ...
            $("#reple"+cnt).append(post_typeDiv, titleDiv, regdateDiv, contentDiv, updateBtnDiv, deleteBtn)
         ...
         <c:forEach var="list" items="${list }" varStatus="cnt" >   
               <div id="reple${cnt.count }" style="width:94%; background-color:#E5E5E1; margin:2%; padding: 1%;">
                   <div>
                      ...
                      <div style="width:100%; display:inline-block;">
                            <c:if test="${list.b_level==0}">제목: </c:if><c:if test="${list.b_level==1}">[답변완료] </c:if>${list.title}
                            <div style="width:90%;">
                               ${list.content}
                            </div>
